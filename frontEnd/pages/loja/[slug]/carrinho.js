...
export default function CarrinhoCliente({ empresaId }) {
  ...
  // Memoriza a URL base da API
  const API_BASE_URL = process.env.NEXT_PUBLIC_EMPRESA_API;
  // TODO: Substituir por um ID de cliente real (obtido do contexto de autenticação)
  const CLIENT_ID = 30;
  ...
  const fetchLoja = useCallback(async () => {
    ...
    try {
      ...
      setLojaAberta(data.aberta || false);
    } catch (error) {
  ...
  // Função memorizada para buscar dados do cliente
  const fetchCliente = useCallback(async () => {
    try {
      const url = `${API_BASE_URL}/clientes/${CLIENT_ID}`;
      const response = await fetch(url);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true);
      await Promise.all([fetchLoja(), fetchCarrinho(), fetchCliente()]);
      setIsLoading(false);
    };

    loadData();
  }, [slug, fetchLoja, fetchCarrinho, fetchCliente]);
  ...
  const aplicarDesconto = useCallback(() => {
    ...
    // Exemplo: 100 pontos = 1% de desconto. Ajuste conforme sua regra de negócio.
    const percentualDesconto = pontosDisponiveis / 100;
    ...
    try {
      ...
      if (ativarFidelidade) {
        if (descontoAplicado > 0) {
          ...
          const urlAtualizaPontos = `${API_BASE_URL}/clientes/${CLIENT_ID}/pontos`;
          ...
          const urlGanharPontos = `${API_BASE_URL}/clientes/${CLIENT_ID}/ganhar-pontos`;

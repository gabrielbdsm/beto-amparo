...
export default function FinalizarPedido({ empresaId }) {
    ...
    const salvarEnderecoCliente = async (clienteId) => {
        console.log("Dados enviados para salvar:", enderecoEntrega);
        const response = await fetch(`${process.env.NEXT_PUBLIC_EMPRESA_API}/clientes/${clienteId}/endereco`, {
            ...
            body: JSON.stringify({
                ...
                destinatario: novoEndereco.destinatario,
                //telefone: novoEndereco.telefone
            })
        });

    console.log("Resposta ao salvar endereço:", response); // 👈 ADICIONAR
...
const handleSalvarEndereco = async () => {
    try {
        ...
        await salvarEnderecoCliente(clienteId); // sua função existente
        setMostrarFormulario(false); // oculta o formulário
    } catch (error) {

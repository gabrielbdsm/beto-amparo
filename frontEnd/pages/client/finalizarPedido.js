...
export default function FinalizarPedido({ empresaId }) {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        // Buscar dados do cliente (mockado - substituir por chamada real)
  ...
  const handleFinalizarPedido = async () => {
    ...
    try {
      const pedidoData = {
        itens: itensCarrinho,
        endereco: enderecos.find(e => e.principal),
        metodoEntrega,
        metodoPagamento,
        cupom,
        total
      ...
      // if (response.ok) router.push(`/${slug}/pedido-confirmado`);
    } catch (error) {
                  ...
                  <div key={endereco.id} className={`border rounded-lg p-4 ${endereco.principal ? "border-blue-500 bg-blue-50" : ""}`}>
                    ...
                    {!endereco.principal && (
                      <button 
                        ...
                        onClick={() => setEnderecos(enderecos.map(e => ({
                          ...e,

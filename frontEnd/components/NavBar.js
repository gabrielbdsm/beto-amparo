...
const NavBar = ({ site }) => {
  ...
  const slug = site || router.query.slug || ''; // fallback para string vazia
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 flex justify-around items-center bg-blue-300 text-white py-2 shadow-inner rounded-t-4xl">
      <Link href={`/${slug}`} className="flex flex-col items-center hover:text-blue-500">
        {/* Ícone Home */}
      ...
      <Link href={`/${slug}/pedidos`} className="flex flex-col items-center hover:text-blue-500">
        <svg
      ...
      <Link href={`/${slug}/carrinho`} className="flex flex-col items-center hover:text-blue-500">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="w-6 h-6 mb-1"
          fill="none"

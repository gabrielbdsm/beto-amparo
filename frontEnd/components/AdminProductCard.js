// components/AdminProductCard.js
...
export default function AdminProductCard({
  ...
  return (
    ...
    >
      {/* Botão de Excluir Permanente (Lixeira) - MANTEMOS A FORMA E O TAMANHO ATUAL */}
      {/* Ele já tem um bom tamanho com 'p-2 rounded-full' */}
      {onPermanentDelete && (
        <button
          onClick={(e) => {
            e.stopPropagation();
            onPermanentDelete(produto);
          }}
      ...
      {/* Ações do Admin (Editar / Ativar-Inativar) */}
      <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-around items-center">
        {onEdit && (
          // Adicionamos 'py-2 px-4' para aumentar a área clicável do botão "Editar"
          <button
            onClick={(e) => { e.stopPropagation(); onEdit(produto.id); }}
            className="flex items-center gap-2 text-blue-600 hover:text-blue-800 font-medium transition-colors duration-200 py-2 px-4 rounded-md"
          >
        ...
        {/* Botão de Ação (Ativar/Inativar) */}
        {onStatusChange && (
          // Adicionamos 'py-2 px-4' para aumentar a área clicável do botão "Inativar/Ativar"
          <button
            onClick={(e) => { e.stopPropagation(); onStatusChange(produto); }}
            className={`flex items-center gap-2 font-medium transition-colors duration-200 py-2 px-4 rounded-md

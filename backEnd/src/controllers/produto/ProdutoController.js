...
export const criarProduto = async (req, res) => {
  try {
    ...
    const id_empresa = req.Id
    
    if (!id_empresa) {
      return res.status(400).json({ mensagem: 'ID da empresa não fornecido.' });
    }
    const {data} = await lojaModel.buscarIdLoja(id_empresa);
    const id_loja = data.id;
    ...
    const URlimagem = await ImageModel.salvarImagem(imagem.buffer, imagem.originalname, imagem.mimetype, id_loja);
...
export const listarProdutosPorLoja = async (req, res) => {
  try {
    ...
    if (!lojaId) {
      return res.status(400).json({ mensagem: "ID da lojaId não fornecido." });
    }

...
// ... (Função deletarProduto mantida - presumindo que exista e não esteja em conflito) ...
...
// --- NOVA FUNÇÃO: Decrementar Estoque ---
...
// NOVA FUNÇÃO: getProduto (busca detalhes específicos de produto por IDs)
export const getProduto = async (ids) => { // Renomeado de getPedido_itens para refletir o que busca
  try {
    const { data, error } = await supabase
      .from('produto')
...
    if (error) {
      console.error('ProdutoModel: Erro ao buscar produtos por IDs:', error.message); // Mensagem de erro mais precisa
      return [];
    }

    return data ?? [];
  } catch (e) {
    console.error('ProdutoModel: Erro inesperado em getProduto:', e); // Nome da função correto no log
    return [];
  }
};